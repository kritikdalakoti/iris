...
export default class DashboardController extends CommonController {
	...
	constructor(server: Hapi.Server) {
		...
		super.serviceImpl = new CommonCrudServiceImpl(
			this.globalVariables.postgres,
			't_dashboard_data',
			't_dashboard_data'
		);
		this.companyMetersServiceImpl = new CommonCrudServiceImpl(
			this.globalVariables.postgres,
			't_company_meters',
			'v_company_meters_data'
		);
		this.influxServiceImpl = new InfluxCrudServiceImpl(
			this.globalVariables.influx
		);
		this.parameterServiceImpl = new CommonCrudServiceImpl(
			this.globalVariables.postgres,
			't_meter_parameters',
			't_meter_parameters'
		);
		this.customReportServiceImpl = new CommonCrudServiceImpl(
			this.globalVariables.postgres,
			't_custom_reports',
			't_custom_reports'
		);
	...
	public async handleGetAllDashboardDataEntries(
		...
		try {
			let size = request.query['size'] ? request.query['size'] : this.size;
			let page = request.query['page'] ? request.query['page'] : this.page;
			let orderString = request.query['sort']
				? request.query['sort']
				: this.sortColumn + ' ' + this.orderBy;

			let condition = await DataPreparation.getNewObject(request.params);
			let filterObj = await DataPreparation.getFilteredData(
			...
			if (request.query['search']) {
				searchQuery = await DataPreparation.getSearchRawQuery(
					this.searchColumnsArray,
			...
			let result = (
				await this.serviceImpl.getAllEntries(
					condition,
					size,
					page,
					orderString,
	...
	public async handleGetAllDashboardDataEntriesForShared(
		request: Hapi.Request,
		h: Hapi.ResponseToolkit
	) {
		let response: Response;
		...
		try {
			...
			responseObj['list'] = finalList;
			responseObj['count'] =
				Object.keys(countQueryResponse['result']).length > 0
					? countQueryResponse['result'][0]['count']
					: 0;
	...
	async getCustomAnalysisData(obj) {
		...
		for (let i = 0; i < parameters.length; i++) {
			...
			meterCondition['meterUniqueSerialNumber'] = meterUsn;
			let meterData =
				await this.companyMetersServiceImpl.getSingleEntryAsObject(
					meterCondition
				);
			let parameterCondition = {};
			parameterCondition['meterId'] = meterData['meterId'];
			parameterCondition['hexAddress'] = hexCode;
			let parameterData =
				await this.parameterServiceImpl.getSingleEntryAsObject(
					parameterCondition
				);
			// let columnName = meterData['name'] + ":" + parameterData['name'];
		...
		if (shiftArrLength > 0) {
			for (let i = 0; i < shiftArrLength; i++) {
				...
				shiftConditionQuery =
					shiftConditionQuery + `shiftId = ${shiftIdArr[i]}`;
			}
			if (
				interval === '3h' ||
				interval === '6h' ||
		...
		for (let i = 0; i < a.length; i++) {
			...
			for (let x = 0; x < aggregatedDataColumns.length; x++) {
				if (
					aggregatedDataColumns[x]['key'] == 'date' ||
					aggregatedDataColumns[x]['key'] == 'time'
				)
					continue;
	...
	async getCustomReportData(reportObj) {
		...
		for (let i = 0; i < length; i++) {
			...
			for (let l = 0; l < operations.length; l++) {
				parametersString =
					parametersString + 'mean("' + key + '") as "mean_of_' + key + '",';
				...
				if (operations[l] == 'max') {
					parametersString =
						parametersString + 'max("' + key + '") as "max_of_' + key + '",';
					...
					parametersString =
						parametersString + 'min("' + key + '") as "min_of_' + key + '",';
		...
		switch (calculatedObj['type']) {
			case 'P1+P2+Pn':
				for (let i = 0; i < calculatedObj['multipleCalculations'].length; i++) {
					...
					console.log(
						'for',
						actualDataObj[param + calculatedObj['multipleCalculations'][i]]
					);
					tempAdd =
						tempAdd +
						actualDataObj[param + calculatedObj['multipleCalculations'][i]];
			...
			case '(P1+P2+Pn)/(P1+P2+Pn)':
				for (let i = 0; i < calculatedObj['array1'].length; i++) {
					...
					addOfArray1 =
						addOfArray1 + actualDataObj[param + calculatedObj['array1'][i]];
				}

				for (let i = 0; i < calculatedObj['array2'].length; i++) {
					console.log('for', actualDataObj[param + calculatedObj['array2'][i]]);
					addOfArray2 =
						addOfArray2 + actualDataObj[param + calculatedObj['array2'][i]];
				}
			...
			case '(P1+P2+Pn)/[sqrt(P1^2+P2^a+Pn)]':
				...
				for (let i = 0; i < calculatedObj['array1'].length; i++) {
					console.log(
						'arra1 loop',
						actualDataObj[param + calculatedObj['array1'][i]]
					);
					addOfArray1 =
						addOfArray1 + actualDataObj[param + calculatedObj['array1'][i]];
				}
				...
				for (let i = 0; i < calculatedObj['array2'].length; i++) {
					...
					for (let j = 0; j < a; j++) {
						powerResult =
							powerResult * actualDataObj[param + calculatedObj['array2'][i]];
					}
			...
			case 'P1+P2':
				console.log(
					actualDataObj[param + calculatedObj['p1']] +
						actualDataObj[param + calculatedObj['p2']]
				);
				return (
					actualDataObj[param + calculatedObj['p1']] +
					actualDataObj[param + calculatedObj['p2']]
			...
			case 'P1+C1':
				return (
					actualDataObj[param + calculatedObj['p1']] +
					parseFloat(calculatedObj['c1'])
				);
			case 'C1+C2':
				return (
					parseFloat(calculatedObj['c1']) + parseFloat(calculatedObj['c2'])
				);
			case 'P1*P2':
				return (
					actualDataObj[param + calculatedObj['p1']] *
			...
			case 'realPower':
				console.log(
					actualDataObj[param + calculatedObj['volt']],
					actualDataObj[param + calculatedObj['current']],
					calculatedObj['pf']
				);

				return (
					1.732 *
					(actualDataObj[param + calculatedObj['volt']] * Math.pow(10, -3)) *
					actualDataObj[param + calculatedObj['current']] *
					actualDataObj[param + calculatedObj['pf']]
				);
			...
			case 'distortionPowerFactor':
				return (
					1 /
					Math.sqrt(
						1 + Math.pow(actualDataObj[param + calculatedObj['thd']], 2)
					)
				);
			...
			case 'reactivePower':
				console.log(
					"actualDataObj[param+calculatedObj['volt']]",
					actualDataObj[param + calculatedObj['volt']],
					"actualDataObj[param+calculatedObj['sCurrent']]",
					actualDataObj[param + calculatedObj['current']],
					"Math.sin(Math.acos(parseFloat(calculatedObj['spf']))",
					Math.sin(Math.acos(actualDataObj[param + calculatedObj['pf']]))
				);
				return (
					1.732 *
					(actualDataObj[param + calculatedObj['volt']] * Math.pow(10, 3)) *
			...
			case 'constantReactivePower':
				console.log(
					...
					actualDataObj[param + calculatedObj['current']],
					"Math.sin(Math.acos(parseFloat(calculatedObj['spf']))",
					Math.acos(Math.cos(pf))
				);
				return (
					1.732 *
					(actualDataObj[param + calculatedObj['volt']] * Math.pow(10, 3)) *
					actualDataObj[param + calculatedObj['current']] *
					Math.sin(Math.acos(parseFloat(calculatedObj['pf'])))
				);
			...
			case 'manualOperation':
				var expression = calculatedObj['expression'];
				var expLowerCase = expression.toLowerCase();
				var checksqrt = expLowerCase.indexOf('sqrt');
				if (checksqrt > -1) {
					expLowerCase = expLowerCase.replace('sqrt', 'Math.sqrt');
				}
				var checkpow = expLowerCase.indexOf('pow');
				if (checkpow > -1) {
					expLowerCase = expLowerCase.replace('pow', 'Math.pow');
				}
				const p1 = actualDataObj[param + calculatedObj['p1']];
				const p2 = actualDataObj[param + calculatedObj['p2']];
				const c1 = parseFloat(calculatedObj['c1']);
				const c2 = parseFloat(calculatedObj['c2']);

				const answer = eval(expLowerCase);
				return answer;
		}
	}
}
