            ...
            return {
                ...
                authenticate: async function (request, h) {
                    ...
                    if (!request.headers.authorization) {
                        ...
                        let serviceImpl = new CommonCrudServiceImpl(globalVariables.postgres, 't_tokens','t_tokens');
                        let userServiceImpl = new CommonCrudServiceImpl(globalVariables.postgres, 't_users','t_users');
                        ...
                        if ( userData.getIsSuccess() == true ) {
                          ...
                          let tempToken: any = (await serviceImpl.getAllEntries(userCondition,10000,0,"token_id DESC","true","true")).getResult()['list'][0]
                          ...
                          //  console.log("temptoken",tempToken['token'] )
                            if ( tempToken['token'] == authorization ) {
                                ...
                                let userResponse = await userServiceImpl.getSingleEntry(condition)
                                let companyId = userResponse.getResult()['companyId'];
                                // console.log(companyId,request.params['companyId'])

                                console.log("request.params['companyId']",request.params['companyId'],"companyId",companyId)
                                if( request.params['companyId'] != undefined) {

                                    if( companyId != request.params['companyId']) {

                                        let response = new Response(false, StatusCodes.UNAUTHORIZED, "UNAUTHORIZED USER", {})
                                        return h.response(response).code(response.getStatusCode()).takeover();    
                                    }  
                                ...
                                let response = new Response(false, StatusCodes.UNAUTHORIZED, CustomMessages.INVALID_AUTH_HEADER, {})
                                return h.response(response).code(response.getStatusCode()).takeover();
                            }
                        }
                        else {
